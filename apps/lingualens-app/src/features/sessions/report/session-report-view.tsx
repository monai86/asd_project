  ...
  return (
    <>
      ...
      <div className="space-y-5">
        ...
        {identityLoaded ? <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center rounded-[var(--radius-card)] bg-[color:var(--color-accent-soft)] text-base font-semibold text-[color:var(--color-accent-strong)]">EL</span>
            <div>
              <h2 className="text-xl font-bold text-ink">{state.childName}</h2>
              <p className="text-slate-600">{state.reportPeriod}</p>
            </div>
          </div>
          <span className={`rounded-[var(--radius-card)] px-4 py-2 text-sm font-bold ${
            isFailedSafety ? "bg-red-100 text-red-700" : "bg-[color:var(--color-accent-soft)] text-clinical"
          }`}>
        ...
        <GlassCard className="p-5">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[var(--radius-card)] border border-line bg-[color:var(--color-surface-reading)] px-3 font-semibold" disabled={!identityLoaded || isStale} onClick={handleCopyLocalDemoShareLink}>
              <Clipboard size={17} aria-hidden="true" /> Copy local demo share link
            </button>
            <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[var(--radius-card)] border border-line bg-[color:var(--color-surface-reading)] px-3 font-semibold" disabled={!identityLoaded || isStale} onClick={handleMarkSent}>
      ...
      <GlassCard className="p-6">
        ...
        <textarea className="mt-5 min-h-80 w-full rounded-[var(--radius-panel)] border border-line bg-[color:var(--color-surface-reading)] p-4 text-sm leading-6 text-ink outline-none focus:ring-2 focus:ring-clinical" value={reportText} readOnly={isEditorLocked} onChange={(event) => {
          if (isEditorLocked) return;
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <button className="inline-flex items-center gap-2 rounded-[var(--radius-card)] border border-line bg-[color:var(--color-surface-reading)] px-4 py-3 text-sm font-bold text-clinical disabled:opacity-50" disabled={!isFinalized} onClick={() => void handleExport("markdown")}><Download size={18} aria-hidden="true" />Export Markdown</button>
          <button className="inline-flex items-center gap-2 rounded-[var(--radius-card)] border border-line bg-[color:var(--color-surface-reading)] px-4 py-3 text-sm font-bold text-clinical disabled:opacity-50" disabled={!isFinalized} onClick={() => void handleExport("html")}><Download size={18} aria-hidden="true" />Export HTML</button>
