...
| Phase 2 | Auth and authorization | Scaffold foundation | Non-mock auth mode now uses a local Supabase JWT verifier scaffold, ignores mock headers, requires configured JWT secret/issuer in production mode, enforces membership, invitation, MFA, and break-glass claim guards, and exposes backend org-admin membership/care-team assignment endpoints. Real Supabase project setup, invite-only signup, MFA enrollment UI, role matrix persistence beyond local JSON, break-glass audit workflow, and frontend auth flows are not implemented. |
